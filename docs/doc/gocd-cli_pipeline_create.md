## gocd-cli pipeline create

Command to CREATE the pipeline with all specified configuration [https://api.gocd.org/current/#create-a-pipeline]

```
gocd-cli pipeline create [flags]
```

### Examples

```
gocd-cli pipeline create sample-pipeline --from-file sample-pipeline.yaml --log-level debug
