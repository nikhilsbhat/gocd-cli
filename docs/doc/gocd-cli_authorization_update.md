## gocd-cli authorization update

Command to UPDATE the authorization configuration present in GoCD [https://api.gocd.org/current/#update-an-authorization-configuration]

```
gocd-cli authorization update [flags]
```

### Examples

```
gocd-cli authorization update --from-file config-repo.yaml -o yaml
...
```
      --create   if a config repo by this name doesn't already exist, run create
