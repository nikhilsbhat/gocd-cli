...
```
  ...
  -i, --ignore strings      ignore the pipelines from 'pipeline show' command
      --path string         path to search for all GoCD pipeline files
  ...
  -f, --pipelines strings   path to GoCD pipeline config file to identify pipeline names
```
