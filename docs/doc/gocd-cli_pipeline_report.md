## gocd-cli pipeline report

Command to GET pipeline report from GoCD [https://sample.gocd.org/go/cctray.xml]

### Synopsis

Command leverages GoCD api [https://sample.gocd.org/go/cctray.xml] to get the latest pipeline reports
available in the GoCD server

```
gocd-cli pipeline report [flags]
```
...
```
      --analyse     if enabled, analyse the report passed over making an API call
      --failed      if enabled, fetches only the pipelines with 'Failure' status
  -h, --help        help for report
      --succeeded   if enabled, fetches only the pipelines with 'Success' status
```
