## gocd-cli artifact update-config

Command to UPDATE artifact config specified configurations in GoCD [https://api.gocd.org/current/#update-artifacts-config]

```
gocd-cli artifact update-config [flags]
```
