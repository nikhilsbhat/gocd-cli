## gocd-cli roles create

Command to CREATE a role with all specified configurations in GoCD [https://api.gocd.org/current/#create-a-gocd-role, https://api.gocd.org/current/#create-a-plugin-role]

```
gocd-cli roles create [flags]
```

### Examples

```
gocd-cli role create sample-config --from-file sample-config.yaml --log-level debug
```
