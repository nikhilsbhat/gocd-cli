## gocd-cli pipeline not-scheduled

Command to GET pipelines not scheduled in last X days from GoCD [/pipelineHistory.json?]

```
gocd-cli pipeline not-scheduled [flags]
```

### Examples

```
gocd-cli pipeline not-scheduled --time 10
```
...
```
      ...
      --from-config-repos          enable this if you need to get the pipeline automatically from config repo, this adds additional calls made to GoCDs
  -h, --help                       help for not-scheduled
