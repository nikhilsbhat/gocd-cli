## gocd-cli server health-messages

Command to see any errors and warnings generated by the GoCD server as part of its normal routine [https://api.gocd.org/current/#get-server-health-messages]

```
gocd-cli server health-messages [flags]
```

### Options

```
  -h, --help   help for health-messages
```
