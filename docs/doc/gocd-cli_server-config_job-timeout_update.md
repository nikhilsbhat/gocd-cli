## gocd-cli server-config job-timeout update

Command to update default job timeout in GoCD server [https://api.gocd.org/current/#update-job-timeout-config]

```
gocd-cli server-config job-timeout update [flags]
```
...
```
  -h, --help          help for update
      --timeout int   default timeout value in minutes for cancelling the hung jobs.
```
