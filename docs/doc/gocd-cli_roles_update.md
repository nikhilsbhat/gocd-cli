## gocd-cli roles update

Command to UPDATE a role with all specified configurations in GoCD [https://api.gocd.org/current/#update-a-role]

```
gocd-cli roles update [flags]
```

### Examples

```
gocd-cli role update sample-config --from-file sample-config.yaml --log-level debug
```

### Options

```
      --create   if a role by this name doesn't already exist, run create
