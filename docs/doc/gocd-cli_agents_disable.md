## gocd-cli agents disable

Command to DISABLE an agent registered with GoCD server [https://api.gocd.org/current/#update-an-agent]

```
gocd-cli agents disable [flags]
```

### Options

```
      --delay duration     time delay between each retries that would be made to get the agent status (default 5s)
  -h, --help               help for disable
      --ids strings        ids of the agent which has to be disabled
      --timeout duration   timeout, if the operation is not successful in this specified duration (default 20m0s)
      --wait               enable this if you want to wait until the agent is disabled completely
```
...
###### Auto generated by spf13/cobra on 24-Dec-2024
