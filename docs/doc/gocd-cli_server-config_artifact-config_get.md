## gocd-cli server-config artifact-config get

Command to get artifact configurations from GoCD server [https://api.gocd.org/current/#get-artifacts-config]

```
gocd-cli server-config artifact-config get [flags]
```
