## gocd-cli roles get-all

Command to GET all the roles present in GoCD [https://api.gocd.org/current/#get-all-roles]

```
gocd-cli roles get-all [flags]
```
...
```
  -h, --help          help for get-all
      --type string   type of role to be fetched, ex: gocd, plugin
```
